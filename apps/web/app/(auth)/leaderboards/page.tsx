...
function calcStreak(dates: Date[]): { current: number; longest: number } {
  ...
  let current = 0;
  if (uniqueDays[0] && (uniqueDays[0].getTime() === today.getTime() || uniqueDays[0].getTime() === yesterday.getTime())) {
    current = 1;
  ...
  for (let i = 1; i < uniqueDays.length; i++) {
    const diff = (uniqueDays[i-1]!.getTime() - uniqueDays[i]!.getTime()) / (1000 * 60 * 60 * 24);
    if (diff === 1) { runLength++; longest = Math.max(longest, runLength); }
    else runLength = 1;
  }
...
export default async function LeaderboardsPage() {
  ...
  const monthlyData = Object.entries(monthlyMap)
    ...
    .map(([key, val]) => {
      ...
      return { month: d.toLocaleDateString("en-US", { month: "short", year: "2-digit" }), total: val.total, rx: val.rx };
    });
  ...
  allSessions.forEach(s => { dayOfWeekCounts[new Date(s.workoutDate).getDay()]++; });
